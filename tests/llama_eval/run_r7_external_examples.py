#!/usr/bin/env python3
"""Run native MVP R7 external-example validation suite."""
...
from tests.llama_eval.r7_external_examples import release_cases

DEFAULT_N_RUNS = 1
MAJORITY_THRESHOLD = 0.5


def _cases() -> list[LiveScenario]:
    ...
    for scenario in scenarios:
        if not scenario_expected_tools_only(
            ...
            raise RuntimeError(
                f"R7 external case contains non-MVP expected tools: {scenario.name}"
            )
    return scenarios
...
def main() -> int:
    ...
    cases = select_cases(_cases(), category=args.category, case_name=args.case)
    if not cases:
