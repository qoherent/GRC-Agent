#!/usr/bin/env python3
"""Run native MVP R4C add-variable eval (expansion track)."""
...
from tests.llama_eval.r4c_add_variable import R4C_ADD_VARIABLE_CASES
...
def release_cases() -> list[LiveScenario]:
    for scenario in R4C_ADD_VARIABLE_CASES:
        if not scenario_expected_tools_only(
            scenario,
            allowed_tool_names=MVP_RELEASE_MODEL_TOOLS,
        ):
            raise RuntimeError(
                f"R4C add_variable case contains non-MVP expected tools: {scenario.name}"
            )
    return list(R4C_ADD_VARIABLE_CASES)
...
def main() -> int:
    parser = build_phase_parser(
        "Native MVP R4C add-variable eval (expansion track).",
