#!/usr/bin/env python3
"""Run native MVP R3 rewire eval (expansion track)."""
...
from tests.llama_eval.r3_rewire import R3_REWIRE_CASES
...
def release_cases() -> list[LiveScenario]:
    for scenario in R3_REWIRE_CASES:
        if not scenario_expected_tools_only(
            scenario,
            allowed_tool_names=MVP_RELEASE_MODEL_TOOLS,
        ):
            raise RuntimeError(
                f"R3 rewire case contains non-MVP expected tools: {scenario.name}"
            )
    return list(R3_REWIRE_CASES)
...
def main() -> int:
    parser = build_phase_parser(
        "Native MVP R3 rewire eval (expansion track).",
