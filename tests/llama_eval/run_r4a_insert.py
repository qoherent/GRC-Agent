#!/usr/bin/env python3
"""Run native MVP R4A insert eval (expansion track)."""
...
from tests.llama_eval.r4a_insert import R4A_INSERT_CASES
...
def release_cases() -> list[LiveScenario]:
    for scenario in R4A_INSERT_CASES:
        if not scenario_expected_tools_only(
            scenario,
            allowed_tool_names=MVP_RELEASE_MODEL_TOOLS,
        ):
            raise RuntimeError(
                f"R4A insert case contains non-MVP expected tools: {scenario.name}"
            )
    return list(R4A_INSERT_CASES)
...
def main() -> int:
    parser = build_phase_parser(
        "Native MVP R4A insert eval (expansion track).",
