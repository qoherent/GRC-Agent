#!/usr/bin/env python3
"""Run native MVP R1 set_state eval (expansion track)."""
...
from tests.llama_eval.r1_set_state import R1_SET_STATE_CASES
...
def release_cases() -> list[LiveScenario]:
    for scenario in R1_SET_STATE_CASES:
        if not scenario_expected_tools_only(
            scenario,
            allowed_tool_names=MVP_RELEASE_MODEL_TOOLS,
        ):
            raise RuntimeError(
                f"R1 set_state case contains non-MVP expected tools: {scenario.name}"
            )
    return list(R1_SET_STATE_CASES)
...
def main() -> int:
    parser = build_phase_parser(
        "Native MVP R1 set_state eval (expansion track).",
