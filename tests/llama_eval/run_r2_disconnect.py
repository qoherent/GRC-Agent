#!/usr/bin/env python3
"""Run native MVP R2 disconnect eval (expansion track)."""
...
from tests.llama_eval.r2_disconnect import R2_DISCONNECT_CASES
...
def release_cases() -> list[LiveScenario]:
    for scenario in R2_DISCONNECT_CASES:
        if not scenario_expected_tools_only(
            scenario,
            allowed_tool_names=MVP_RELEASE_MODEL_TOOLS,
        ):
            raise RuntimeError(
                f"R2 disconnect case contains non-MVP expected tools: {scenario.name}"
            )
    return list(R2_DISCONNECT_CASES)
...
def main() -> int:
    parser = build_phase_parser(
        "Native MVP R2 disconnect eval (expansion track).",
