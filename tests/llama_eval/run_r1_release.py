#!/usr/bin/env python3
"""Run native MVP R1 release eval (mutation operations).

Run:
    uv run python -m tests.llama_eval.run_r1_release --quick
    uv run python -m tests.llama_eval.run_r1_release --n-runs 3 --results-path /tmp/r1.json
"""
...
from tests.llama_eval.r1_release import R1_CASES
...
def main() -> int:
    parser = build_phase_parser(
        "Native MVP R1 release eval: mutation operations.",
