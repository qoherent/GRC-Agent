#!/usr/bin/env python3
"""Run native MVP R4B remove-block eval (expansion track)."""
...
from tests.llama_eval.r4b_remove import R4B_REMOVE_CASES
...
def release_cases() -> list[LiveScenario]:
    for scenario in R4B_REMOVE_CASES:
        if not scenario_expected_tools_only(
            scenario,
            allowed_tool_names=MVP_RELEASE_MODEL_TOOLS,
        ):
            raise RuntimeError(
                f"R4B remove case contains non-MVP expected tools: {scenario.name}"
            )
    return list(R4B_REMOVE_CASES)
...
def main() -> int:
    parser = build_phase_parser(
        "Native MVP R4B remove eval (expansion track).",
