...
from grc_agent.cli import _collect_package_paths, _run_paths_command
...
class PathsCommandTests(unittest.TestCase):
    ...
    def test_json_output_is_valid_json(self) -> None:
        ...
        self.assertIn("llama_logs", payload)
    ...
    def test_paths_are_absolute(self) -> None:
        """All paths should be absolute and under the user's home directory."""
        ...
        for key, value in paths.items():
            ...
            # Most user paths live under $HOME; the only common exception is
            # `/tmp`-style overrides, which we don't produce here.
            self.assertTrue(
                str(value).startswith(str(home)),
                f"{key} = {value!r} does not live under $HOME",
            )
