...
Usage:
    ...
    uv run python -m tests.real_usage.run_real_usage_v1 --graph 6
"""
...
def _score_task(task: RealUsageTask, result: TaskResult) -> bool:
    if task.expect_refusal:
        if "apply_edit" in result.executed_tool_names:
            return False
        if "unsupported" not in result.assistant_text.lower():
            return False
        return True
...
def main() -> None:
    ...
    for gi, (graph_id, graph_path) in enumerate(graphs.items()):
        ...
        for ti, task in enumerate(graph_tasks):
            label = f"  [{ti+1}/{task_count}] {task.task_id}: {task.name}"
