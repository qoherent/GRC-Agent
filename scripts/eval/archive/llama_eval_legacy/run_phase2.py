...
PHASE2_CASES: list[ChainCase] = [
    ...
    ChainCase(
        ...
        "Show me how blocks_throttle2_0 is wired, then preview removing the connection from the source to blocks_throttle2_0.",
...
def _render_run_status(case: ChainCase, run_result: dict[str, Any]) -> str:
    status = "PASS" if run_result["sequence_matched"] else "FAIL"
    return f"{status} ({', '.join(run_result['tools_called']) or 'no tools'})"
...
def main() -> int:
    parser = build_phase_parser(
        "Phase 2 model eval: ordered multi-tool chains.",
