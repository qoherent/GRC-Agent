...
from grc_agent.config import AppConfig, ConfigError, load_app_config, resolve_config_path
...
def _check_gnuradio_import() -> dict[str, Any]:
    ...
    ok = gnuradio_version == EXPECTED_GNURADIO
    detail = f"{gnuradio_version} (expected {EXPECTED_GNURADIO})"
    return _build_check("GNU Radio import/version", ok, detail, version=gnuradio_version)
