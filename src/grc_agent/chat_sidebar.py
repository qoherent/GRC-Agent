...
class ChatSidebar(
    StreamViewMixin, TranscriptViewMixin, ComposerMixin, ApprovalsMixin, Gtk.Box
):
    ...
    send), and ``ApprovalsMixin`` (the approval gate) are U15 splits; each
    shares this one instance's attributes by convention, not encapsulation —
    an organizational split of a genuinely single widget.
    """
