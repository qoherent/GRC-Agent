...
from .chat.images import _thumbnail
from .db import (
    archive_transcript,
...
class ChatSidebar(StreamViewMixin, TranscriptViewMixin, Gtk.Box):
    ...
    ``StreamViewMixin`` (live-streaming render) and ``TranscriptViewMixin``
    (rendering completed turns) are U15 splits; each shares this one
    instance's attributes by convention, not encapsulation — an
