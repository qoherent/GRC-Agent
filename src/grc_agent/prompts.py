...
def build_system_prompt(session_id: str | None = None) -> str:
    ...
    return prefix + (
        ...
        "- Standard C++ catalog blocks are the default: always search the block catalog (query_knowledge) before creating "
        "an epy_block. Use an Embedded Python Block only when no native block or combination of blocks can perform the DSP/logic.\n"
        "- When writing custom logic in an epy_block, use vectorized NumPy/SciPy array operations (e.g. out[:] = np.sin(...)) "
        "in work() rather than scalar Python loops to maintain streaming performance.\n"
        ...
        "- If logs indicate TUN/TAP allocation failures (tun_alloc, TUNSETIFF, or EPERM allocating a tun/tap interface), the cause is missing "
        "CAP_NET_ADMIN, not a graph defect: creating a named interface needs it, and a world-accessible /dev/net/tun does not substitute. "
        "Advise the user to run once, outside the app: `sudo ip tuntap add dev tap0 mode tap user $USER` (match the block's ifname and mode: "
        "tap for TAP/Ethernet frames, tun for TUN/IP packets) — the "
        "flowgraph then attaches to the pre-created interface without privileges. The interface persists across flowgraph restarts but not "
        "reboots: re-create it after each reboot or make it durable with a systemd unit. Do not advise running the app as root or "
        "setcap-ing the interpreter (same posture as SDR USB errors).\n\n"
...
def build_planner_prompt(session_id: str | None = None) -> str:
    ...
    return prefix + (
        ...
        "- Default to standard GNU Radio C++ catalog blocks; specify Embedded Python Blocks only for custom protocols or state machines where native blocks do not exist.\n"
