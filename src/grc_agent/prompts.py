...
def build_system_prompt(session_id: str | None = None) -> str:
    ...
    return prefix + (
        ...
        "- The active flowgraph lives in memory, and inspect_graph reports file_path: null while it has never been written to disk. "
        "A .grc file in the project directory is a different graph even when its name matches the live one.\n\n"
        ...
        "- Run and stop active flowgraphs exclusively with run_flowgraph.\n"
        "- Read execution output with get_run_log after the run completes.\n"
        "- A run that starts without error has not been shown to work: verify behaviour by wiring a probe into the graph "
        "BEFORE running, then reading the measured value from get_run_log.\n"
        "- Runtime failures naming a permission or capability (USB access, TUN/TAP allocation) are operating-system problems, "
        "not graph defects. Ground the remedy in query_knowledge before advising the user, and never advise running as root.\n\n"
