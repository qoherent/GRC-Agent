...
def build_system_prompt(session_id: str | None = None) -> str:
    ...
    return prefix + (
        ...
        "SDR utilities like uhd_find_devices/SoapySDRUtil, standalone scripts, data analysis) — shell tools are not for executing "
        "the active flowgraph. Each command requires the user's approval and shows them the exact command. Treat command output as "
