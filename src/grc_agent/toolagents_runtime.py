...
# Platform-agnostic addendum for Ollama specifically — no `systemctl` /
# `journalctl` / Linux-specific text, since Ollama is a menu-bar/Start-menu
# desktop app on macOS/Windows. 120000 matches this app's own default model
# (gemma4:e4b-it-qat-120k's baked-in num_ctx) — a concrete, known-good number
# rather than a vague "set it higher". Deliberately does NOT say "start it
# with `ollama serve <env>`": on a standard install Ollama is already
# running in the background, and a second manually-started instance
# conflicts with it (port already bound) instead of fixing anything — the
# context size has to be set as a *persistent* setting on the server that's
# already running, not a one-off command. Exact per-OS steps live in this
# app's README rather than duplicated here.
OLLAMA_SERVER_HINT = (
    "If using Ollama: it's usually already running in the background after "
    "install — don't start a second one manually, it will just conflict "
    "with the running one. This app needs a much larger context window than "
    "Ollama's small default. Set OLLAMA_CONTEXT_LENGTH=120000 as a "
    "persistent setting on the Ollama server (see this app's README's 'LLM "
    "backend' section for the exact steps on your OS), or bake a large "
    "num_ctx into a custom Modelfile for one specific model instead."
)


def _backend_unreachable_hint(server_url: str, backend: str = "") -> str:
    ...
    """
    hint = f"Connection refused at {server_url}."
    if backend == "ollama":
        hint = f"{hint} {OLLAMA_SERVER_HINT}"
    return hint
...
# Same situation, but the wire response's own finish_reason says why: the
# generation was cut off, not chosen to stop (AGENTS.md "No Assumed
# Reasoning Failures" — this is a harness/config fact, not a verdict on the
# model). Native-derived from finish_reason == "length"; never guessed from
# text content.
_TRUNCATED_MODEL_RESPONSE_TEXT = (
    "No response was generated by the model (finish_reason=length: the "
    "generation was cut off, not completed). This usually means the "
    "conversation has exceeded the model's loaded context window. Check the "
    "model's actual context length with `ollama ps`."
)


# Factual note injected (user-role, runtime-directive wrapped) when the model
# re-sends a byte-identical failing batch, or repeatedly hits the same kind
# of failure with varying arguments. Purely informational — no behavioral
# command or procedural recipe (AGENTS.md "no in-band control flow"); the
# system prompt remains the only behavioral authority.
_LOOP_NOTE = (
    "<runtime_directive>\n"
    "Recent tool calls this turn have repeatedly failed the same way — either "
    "with identical arguments, or with varying arguments that hit the same "
    "underlying error. Repeating the same approach will not change the "
    "result; the actual issue named in the error needs to be addressed.\n"
    "</runtime_directive>"
)
...
# A second, more lenient detector for a model churning through many
# superficially-different failing calls that all fail the SAME way (same
# tool + same error code) without ever resolving it — evades the exact-
# argument detector above by varying an instance name or malformed string
# each time. Since a failed change_graph call never mutates the graph (the
# whole batch is rolled back), "same error code repeated" is itself already
# evidence of zero progress; no separate graph-state check is needed. A
# higher threshold than the exact-match one since a shared error *code*
# (e.g. 'gnu_validation_failed' covers many distinct native validation
# issues) is a broader net and needs more repetition before it's reliably a
# stuck loop rather than a few coincidentally-same-category mistakes.
_CATEGORY_LOOP_WARN_THRESHOLD = 4
_CATEGORY_LOOP_STOP_THRESHOLD = 6
...
def _failure_code(result: Any) -> str | None:
    """The representative failure category for one failed tool result.

    Prefers the top-level ``error_type``; falls back to the first entry's
    ``errors[].code`` when a result carries errors without a top-level
    ``error_type`` (e.g. some ``change_graph`` validation payloads). One
    uniform extraction for every tool, not a per-tool special-case.
    """
...
def _apply_turn_counters(
    ...
    *,
    ...
    """Attach the turn-result counter fields shared by every turn outcome payload."""
    payload["model"] = model
    payload["steps"] = steps
...
def _model_message_event(role: str, message: ChatMessage) -> dict[str, Any]:
    """The ``model_message`` event wrapping one already-built ``ChatMessage``."""
    return {"event": "model_message", "role": role, "payload": chat_message_payload(message)}


def _last_message_event(agent: GrcAgent, role: str) -> dict[str, Any]:
    """``model_message`` event for the message most recently added to ``agent.chat_history``."""
    return _model_message_event(role, agent.chat_history.get_messages()[-1])
...
def _backend_unreachable_payload(
    *,
    ...
    hint = _backend_unreachable_hint(server_url, backend)
...
class GrcResponseConverter:
    ...
    def from_provider_response(self, response_data: Any) -> ChatMessage:
        ...
        if hasattr(response_data, "choices") and response_data.choices:
            ...
            if finish_reason:
                chat_message.additional_information["finish_reason"] = finish_reason
            usage = getattr(response_data, "usage", None)
            if usage is not None:
                chat_message.additional_information["usage"] = (
    ...
    def yield_from_provider(self, stream_generator: Any) -> Generator[Any, None, None]:
        """Filter empty-``choices`` chunks and capture what they discard.

        Ollama's OpenAI-compat endpoint sends token usage on a final chunk
        with empty ``choices`` (the crash this filter exists to prevent —
        see the class docstring's sibling fix). The parent converter's own
        ``finished_chat_message`` assembly pops ``choices`` off the chunk
        that carries ``finish_reason``, discarding it before anything else
        can read it. Both values are captured here, at the boundary of our
        own wrapper, and merged onto the finished message afterward — no
        monkeypatching of the parent converter's internals.
        """
        ...
        for streamed in self.parent_converter.yield_from_provider(safe_stream()):
            if streamed.finished and streamed.finished_chat_message is not None:
                streamed.finished_chat_message.additional_information.update(captured)
            yield streamed
...
class ToolAgentsLlamaProviderConfig:
    ...
    def create_settings(
        ...
        *,
        ...
        No ``max_tokens`` is set: per AGENTS.md ("Maximizing Context & No
        String-Based Clipping"), the backend's own maximum output capacity
        is used rather than an arbitrary generation cap that can truncate
        a model mid-reasoning before it emits its tool call.
        """
...
class ToolAgentsToolDelegate:
    def __init__(
        ...
        *,
        ...
        on_tool_rejected: Callable[[str, dict[str, Any], Any], None] | None = None,
    ) -> None:
        self.agent = agent
        self.tool_name = tool_name
        self.on_tool_start = on_tool_start
        self.on_tool_end = on_tool_end
        self.on_tool_rejected = on_tool_rejected
    ...
    def invoke(
        ...
        *,
        ...
        validation_result = (
            ...
            else self.agent.validate_tool_call(self.tool_name, normalized)
        )
        if validation_result is not None:
            if self.on_tool_rejected:
                try:
                    self.on_tool_rejected(self.tool_name, normalized, validation_result)
                except Exception as e:
                    logger.error(f"Error in on_tool_rejected callback: {e}")
    ...
    def build(
        ...
        for schema in self.agent.get_tool_schemas_for_turn(allowed_tool_names):
            ...
            delegate = ToolAgentsToolDelegate(
                ...
                on_tool_rejected=self.on_tool_rejected,
            )
...
class ToolAgentsRunner:
    ...
    def _evaluate_loop_state(
        ...
        failing_call_categories: dict[tuple[str, str], int],
    ) -> _LoopDecision:
        """Return CONTINUE / WARN / STOP for one tool call.

        Mutates ``failing_call_signatures`` and ``failing_call_categories``
        in place. Two layered detectors: the tight exact-argument-repeat
        check (unchanged), and a looser same-error-category check that
        catches a model varying its arguments (a different instance name,
        a differently-malformed string) each time while making the same
        underlying mistake — a KeyError/rejection loop that never trips the
        exact-match check because no two calls are byte-identical.
        """
        ...
        category_decision = _LoopDecision.CONTINUE
        code = _failure_code(result)
        if code is not None:
            cat_key = (tool_name, code)
            failing_call_categories[cat_key] = failing_call_categories.get(cat_key, 0) + 1
            cat_count = failing_call_categories[cat_key]
            if cat_count == _CATEGORY_LOOP_WARN_THRESHOLD:
                category_decision = _LoopDecision.WARN
            elif cat_count >= _CATEGORY_LOOP_STOP_THRESHOLD:
                category_decision = _LoopDecision.STOP

        if count >= _LOOP_STOP_THRESHOLD or category_decision is _LoopDecision.STOP:
            return _LoopDecision.STOP
        if count == _LOOP_WARN_THRESHOLD or category_decision is _LoopDecision.WARN:
        ...
        for attempt in range(retry_attempts):
            ...
            try:
                for _retry_state in Retrying(
                    ...
                    with _retry_state:
                        ...
                        if cancelled:
                            ...
                            yield _model_message_event(ASSISTANT_MODEL_ROLE, assistant_message)
                            return None
                        ...
                        if not tool_calls and not assistant_text:
                            if attempt < retry_attempts - 1:
                                finish_reason = assistant_message.additional_information.get(
                                    "finish_reason"
                                )
                                logger.warning(
                                    "Degenerate empty response with no tool calls "
                                    "(attempt %d/%d, finish_reason=%s). Retrying...",
                                    attempt + 1,
                                    retry_attempts,
                                    finish_reason,
                                )
                                # Otherwise this attempt vanishes with no trace anywhere
                                # a saved transcript can show it — only the log line
                                # above, which isn't persisted with the run's output.
                                yield {
                                    "event": "degenerate_retry",
                                    "attempt": attempt + 1,
                ...
                payload = _backend_unreachable_payload(
                    ...
                    backend=self.provider_config.backend,
                )
                agent.chat_history.add_assistant_message(payload["assistant_text"])
                yield _last_message_event(agent, ASSISTANT_MODEL_ROLE)
    ...
    def _dispatch_tools(
        ...
        *,
        ...
        for tool_call in tool_calls:
            ...
            if delegate is None:
                # A malformed emission can corrupt the tool name itself (the
                # provider-side parser leaking argument fragments into the
                # name field), e.g. "change_graph\n<arg_key>add_blocks...".
                # Detected structurally (name starts with a real tool name
                # but isn't equal to it) — never a hardcoded pattern for one
                # specific corruption string — so the model is told its call
                # syntax is broken, not that the tool doesn't exist.
                garbled_match = next(
                    (name for name in active_allowed_tools if tool_name.startswith(name)),
                    None,
                )
                if garbled_match:
                    result = build_error_payload(
                        ...
                        message=(
                            f"Malformed tool call: the tool name {tool_name!r} has extra "
                            f"content appended after {garbled_match!r}. Resend the call "
                            f"with the tool name exactly {garbled_match!r} and the "
                            "arguments in the arguments object, not appended to the name."
                        ),
                    )
                else:
                    result = build_error_payload(
                ...
                executed = False
                if registry_builder.on_tool_rejected:
                    raw_args = tool_call.tool_call_arguments
                    try:
                        registry_builder.on_tool_rejected(
                            tool_name, raw_args if isinstance(raw_args, dict) else {}, result
                        )
                    except Exception as e:
                        logger.error(f"Error in on_tool_rejected callback: {e}")
            else:
            ...
            yield _model_message_event(TOOL_MODEL_ROLE, tool_result_message)
            ...
            if decision is _LoopDecision.WARN:
                ...
                yield _model_message_event(USER_MODEL_ROLE, note_msg)
            elif decision is _LoopDecision.STOP:
                loop_stuck = True
                break

        if loop_stuck:
            stuck_text = "(stopped: the same kind of tool call failure repeated multiple times in a row)"
            payload = build_error_payload(
                error_type=ErrorCode.SAFETY_CEILING,
                message=(
                    "Stopped: the same kind of tool call failure repeated multiple times in a row."
                ),
            )
            _apply_turn_counters(
                payload,
                model=resolved_model,
                steps=assistant_turns,
                tool_rounds_used=tool_rounds_used,
                tool_calls_requested=len(tool_calls),
                tool_calls_executed=tool_calls_executed + round_executed,
    ...
    def _run_turn_events(
        ...
        *,
        ...
        yield _last_message_event(agent, USER_MODEL_ROLE)
        ...
        failing_call_signatures: dict[tuple[str, str], int] = {}
        # Per-(tool, error-code) count of same-category failing calls this turn.
        failing_call_categories: dict[tuple[str, str], int] = {}
        ...
        while True:
            ...
            if tool_calls:
                ...
                outcome = yield from self._dispatch_tools(
                    ...
                    failing_call_categories=failing_call_categories,
            ...
            if empty_terminal:
                truncated = assistant_message.additional_information.get("finish_reason") == "length"
                assistant_text = (
                    _TRUNCATED_MODEL_RESPONSE_TEXT if truncated else _EMPTY_MODEL_RESPONSE_TEXT
                )
            ...
            final_result: dict[str, Any] = _apply_turn_counters(
            ...
            final_finish_reason = assistant_message.additional_information.get("finish_reason")
            if final_finish_reason:
                final_result["finish_reason"] = final_finish_reason
            final_usage = assistant_message.additional_information.get("usage")
            if final_usage:
                final_result["usage"] = final_usage
            if empty_terminal:
