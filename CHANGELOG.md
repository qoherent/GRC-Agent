...
Harness lean-out and model-facing tool contract rework, planned in [`docs/plans/2026-09-02-0830-refactor-harness-lean-and-tool-contracts-plan.md`](docs/plans/2026-09-02-0830-refactor-harness-lean-and-tool-contracts-plan.md) and [`docs/plans/2026-09-03-0829-refactor-sidebar-decomposition-review-plan.md`](docs/plans/2026-09-03-0829-refactor-sidebar-decomposition-review-plan.md). Easy-wins backlog cleanup pass, planned in [`docs/plans/2026-09-08-1101-refactor-easy-wins-backlog-cleanup-plan.md`](docs/plans/2026-09-08-1101-refactor-easy-wins-backlog-cleanup-plan.md). Session-165 behavioral post-mortem and its harness fixes, audited in [`docs/investigation/audit-session-165-approval-crash-and-graph-identity.md`](docs/investigation/audit-session-165-approval-crash-and-graph-identity.md) and planned in [`docs/plans/2026-09-08-1340-fix-session-165-harness-grounding-plan.md`](docs/plans/2026-09-08-1340-fix-session-165-harness-grounding-plan.md).
...
- **Approval card crashed on JSON-stringified `change_graph` arguments**: pydantic-ai keeps `before_tool_validate`'s repair in a local (`tool_manager.py:378-380`) and builds `DeferredToolRequests` from the original `ToolCallPart`, so the card rendered the model's raw args — `{"add_blocks": "[{…}]"}` raised `AttributeError: 'str' object has no attribute 'get'` (a str iterates as characters) and the connection families rendered one bullet per character, asking the user to approve a change they could not read. The repair rule now lives in one place (`json_args.repair_json_args`), applied by the capability for validation and by the card for rendering; the card renders a still-non-mapping element literally rather than dropping or raising; and the rule returns a new mapping, so the recorded history keeps the arguments the model actually emitted (`raw_args` is `call.args` itself, so the previous in-place loop rewrote history whenever a provider sent dict args). `change_graph` was the only approval-gated tool with nested list-of-dict args, hence the only one on that path. The validation contract is unchanged: only the schema-free renderer decodes stringified list *elements*, because loosening the schema-driven path would make arguments the tool contract currently rejects start validating — and that strictness is a recorded decision (`docs/backlog.md`, "Scenario suite default model").
- **The live flowgraph had no identity the model could read**: `inspect_graph` emitted no path or saved flag, `graph_name` is only the options block id (`"default"` for a fresh page, from GRC's own `default_flow_graph.grc`), and the one dynamic instruction that named the active file returned `None` exactly when the graph was unsaved — so an unsaved buffer and a same-named 14,759-byte `.grc` in the project directory were indistinguishable, and the agent concluded the file *was* the live graph. Payloads now carry `file_path` (null = never written to disk) from GRC's own `flow_graph.grc_file_path`, the instruction hook always returns a line and reads the path through the declared deps protocol instead of probing a private canvas attribute, and both prompts state the live-versus-disk contract.
- **`.grc` reads silently discarded `offset`/`limit`**: the `.grc` branch returned before the paging arguments were read while the schema advertised them unconditionally, so three reads at limit 50/10/5 returned byte-identical payloads with no signal. The drop is now disclosed in the header that already names the engine and source, and the parameter descriptions say a structural view has no lines.
- **Client-side turn failures left no record, and the recovery path destroyed the evidence**: a run that ends in an approval request completes legitimately (`DeferredToolRequests` is a declared output), so a crash in the card path produced a DB with `run_completed` and no failure at all — while the salvage popped the trailing response carrying the offending call and re-saved over it, and no step-store snapshot existed for that response because `StepPersistence` gates snapshots on `is_provider_valid`. Every client-side turn failure now archives its at-failure history as a `turn_failure` run, captured before the salvage, with one `run_failed` event carrying the exact text the user saw.
- **Fabricated session paths for unsaved tabs**: `save_session` resolved whatever it was handed, turning the sidebar's `untitled:<tab title>` sentinel into an absolute path that never existed and leaving every chat held on an unsaved tab permanently unreachable (Recent filters on `exists()`). Canonicalisation moved to the only layer that knows it holds a filesystem path; the store writes what it is given. Reopenability of pathless sessions remains deferred.
- **Orphaned `run_tools` table dropped**: 140 rows from an older pydantic-ai-harness, with no writer in the source or in git history, not created by harness 0.28.0, read by nothing, and reached by no sweep or per-session delete. `init_db` drops it idempotently; the harness store's own `schema_version` is untouched.
...
- **Test-only history helper**: Deleted `chat/history.py`'s `_messages_call_tool` and its test — the plan handoff reads the durable plan store (`_show_implement_plan_if_ready` → `load_plan_items`), so nothing in `src/` had called it since that refactor.
- **Orphaned `run_tools` table**: Dropped from the session database (see Fixed).
