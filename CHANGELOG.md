...
- `ResilientSummarizingCompaction` and a manual `compact_now` toolbar button: summarizes aging turns when approaching context limits while preserving all user prompts (`keep_user_messages=True`) and degrading gracefully (history unchanged) if summarization fails — verified against the harness source, a summary failure escalates to the zero-LLM sliding-window tier instead of failing the turn.
