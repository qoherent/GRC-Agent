...
- Adversarial-audit hardening of the fs tools: nested secret/config files denied (`**/.env*`, `**/.grc_agent/*`, `**/.git/*` alongside the root forms), `.grc` detection made case-insensitive and backup-suffix-aware (`.GRC`, `.grc~` no longer leak raw flowgraph source), the write gate re-checked against the symlink-resolved target (in-root `alias.py` → `real.grc` bypass closed), and malformed-`.grc` retries carry fixed text only (parse exceptions can interpolate file content and retry text is not injection-classified).
