...
- Indirect prompt-injection defense (`PromptInjectionDefender`, `stackone-defender` tier-1 patterns): every client-executed tool result is classified; high/critical-risk results are withheld and replaced with a short notice, flags logged. Live-verified end-to-end: an injection payload planted in a project file read via `read_file` never reaches the model.
- Adversarial-audit hardening of the fs tools: nested secret/config files denied (`**/.env*`, `**/.envrc`, `**/.grc_agent/*`, `**/.git/*` alongside the root forms), `.grc` detection made case-insensitive and backup-suffix-aware (`.GRC`, `.grc~` no longer leak raw flowgraph source; a missing `.grc` now reports File-not-found instead of a misleading parse error), the write gate re-checked against the symlink-resolved target (in-root `alias.py` → `real.grc` bypass closed), and malformed-`.grc` retries carry fixed text only (parse exceptions can interpolate file content and retry text is not injection-classified). Triple-verification pass (third independent auditor) confirmed every prior finding closed with live repros and found no regressions.
