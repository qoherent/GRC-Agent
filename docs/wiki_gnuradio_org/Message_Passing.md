...
The message passing interface heavily relies on Polymorphic Types (PMTs) in GNU Radio. For further information about these data structures, see the page Polymorphic Types (PMTs) "Polymorphic Types \(PMTs\)").
...
In the previous flow graph, we have a block called _PDU to Tagged Stream_. A PDU (protocol data unit) in GNU Radio has a special PMT type, it is a pair of a dictionary (on CAR) and a uniform vector type. See Polymorphic_Types_(PMTs)#Pairs#Pairs "Polymorphic Types \(PMTs\)"). So, this would yield a valid PDU, with no metadata and 10 zeros as stream data:
