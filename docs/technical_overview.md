...
Every payload names the graph it describes: `file_path` carries GRC's own `grc_file_path` and is `null` when the graph has never been written to disk. `graph_name` is only the options-block `id` (`default` for a fresh page, from GRC's `default_flow_graph.grc`), which coincides with a filename merely because saving renames the id to the file stem — so an unsaved live graph and a same-named `.grc` in the project directory are distinguished by `file_path`, not by name (session-165 audit).
...
- **Lexical Fallback**: Hybrid RRF fusion is primary on a healthy llamacpp DB (both the vec0 and FTS5 indexes exist there, so the two rankings fuse via Reciprocal Rank Fusion and `search_mode` reads `"hybrid"`). If the embedding call itself fails (backend unreachable, model not pulled), `query_knowledge` transparently falls back to a local SQLite FTS5 (BM25) keyword search over the same catalog/docs corpus — including on a cold cache where the embedding backend was never reachable, in which case the DB is built lexical-only (no vector index) rather than failing the whole ingest. The tool result always tags which path served it (`"search_mode": "vector" | "lexical" | "hybrid"`), so a fallback is never silent to the caller.
...
### 5. Native Flowgraph Execution (`run_flowgraph`)

The agent runs and stops the active flowgraph through GRC's **native Execute/Stop actions** via [`run_flowgraph(action='start'|'stop', wait=True, timeout_seconds=60.0, stop_after_seconds=...)`](../src/grc_agent/agent.py) — the exact toolbar path: GRC generates from the live in-memory graph, spawns the process with its own interpreter, and streams output to the GRC console where the user watches it live. The tool returns status only (started / completed / stopped_after_timeout / still_running / not_started, plus the return code); the full output is read back through `get_run_log`, which always reports `run_in_progress` so the model can tell a live run's log from the previous run's. Running (`action='start'`) is approval-gated (`requires_approval=True`, same deferred-tool mechanism as `change_graph`) because it may transmit RF on connected hardware; stopping (`action='stop'`) is the safe direction and ungated. Bounded runs (`stop_after_seconds`) allow an autonomous runtime budget where the flowgraph is stopped automatically after the specified duration. Pre-gates replicate GRC's own handler conditions (page open, not already running, saved file path, `validate()` + `is_valid()`) because a disabled `Gio` action is a silent no-op and an unsaved page would open a modal Save-As that blocks the unified loop. GUI flowgraphs run until stopped: `wait=False` returns immediately and the model polls `get_run_log` / calls `run_flowgraph(action='stop')` — making the probe → run → read-log verification loop fully autonomous in one turn. Failures of agent-initiated runs are reported by the tool result itself; the automatic failure-notification turn is suppressed for those (the monitor's consume-once `agent_initiated` flag), while user-initiated runs keep the notification.
...
`change_graph` is registered with pydantic-ai's `requires_approval=True`, so the model's call is **never executed until the user consents** — the run ends with a `DeferredToolRequests` output and the sidebar renders one `ApprovalCard` per proposed call: the model's required one-line `reason`, a uniform structured summary of the change (Markdown bullets — no raw JSON), and Approve / Deny / Always-accept actions. The card renders the model's arguments **pre-validation** — pydantic-ai keeps `before_tool_validate`'s repair in a local and builds the deferred request from the original `ToolCallPart` — so it applies the same shared rule (`json_args.repair_json_args`) itself and renders any element that is still not a mapping literally; it can no longer raise, because it is the last step before human consent. The same turn then resumes via `agent.iter(deferred_tool_results=...)` with `ToolApproved()`/`ToolDenied(message)`; a denial is fed back to the model natively and the prompt forbids re-submitting a denied edit. The gate persists in `.env` (`GRC_AGENT_APPROVE_CHANGES` = `manual` default | `auto` | `yolo`) and is controlled from the composer's `Mode` button (Manual: ask all; Auto: flowgraph auto-applied & shell asks; YOLO: all actions un-gated). Because the tool body never runs before consent, there is no unapproved in-memory mutation for the 1.5s safety-net poll to auto-write.

#### Header-Band / Flow-Band Full Relayout
Since the LLM lacks spatial awareness, block positioning is resolved programmatically — and, whenever a `change_graph` batch **changes topology** (any of add/remove blocks or connections; one uniform rule since 2026-08-24, previously only `add_blocks`), the *whole* flowgraph (not just the new block) is relaid out from scratch, so a later wire-only call re-ranks blocks that were added unwired instead of freezing a stale alphabetical stack. Every block is classified into a header band (variables, the options block, imports, snippets — packed left-to-right in fixed-width rows, alphabetically sorted, the options block always pinned first) or a flow band (everything else, laid out left-to-right by topological rank via `grandalf`, one grid cell per block; each weakly-connected component gets its own row band so independent chains never interleave, and same-rank blocks are ordered by a bounded barycenter crossing-minimizer — 8 sweeps — over the component's upstream positions, computed once per batch in a shared `LayoutModel`). Coordinates snap to a shared grid (`BLOCK_FOOTPRINT_W=200`, `BLOCK_FOOTPRINT_H=168`, `BLOCK_SPACING=32`) so overlap detection stays consistent across both bands. This only ever runs from `change_graph`'s own mutation — a user's manual canvas edits are never touched by it.
