...
def resolve_auto(
    ...
    if not is_add_phase:
        ...
        if len(live_dtypes) == 1:
            ...
            raise ValueError(f"Auto-resolution conflict: multiple neighbor types found {live_dtypes}")
    ...
    if add_connections:
        for conn_str in add_connections:
            ...
            try:
                ...
                ports = other_block.active_sources if own_direction == "inputs" else other_block.active_sinks
                for prt in ports:
...
def keep_param(
    ...
    is_port_count_controlling = param_key in port_count_controlling_params(block.key)
    is_structural_enum = is_type_controlling or is_port_count_controlling or param_key == "generate_options"

    if hide == "part" and not is_structural_enum:
        is_custom = value != default
        is_var_ref = variable_names and any(tok in variable_names for tok in _IDENTIFIER_RE.findall(value))
...
def inspect_graph(flow_graph: Any, targets: list[str] = None, view: str = "overview") -> dict[str, Any]:
...
def change_graph(flow_graph: Any, 
    ...
    if not any([add_blocks, remove_blocks, update_params, update_states, add_connections, remove_connections]):
        return {
            "ok": False,
    ...
    if not valid and not force:
        ...
        for elem, msg in flow_graph.iter_error_messages():
            ...
            if parent is not None and parent is not elem:
                validation_errors.append({
                    "code": "gnu_validation",
                    "message": f"{parent.name}: {elem}: {msg}"
                })
            else:
                validation_errors.append({
                    "code": "gnu_validation",
        ...
        return {
            ...
            "errors": validation_errors if validation_errors else [{"code": "gnu_validation", "message": "GRC validation failed."}]
        }
    ...
    try:
        ...
        return {
            ...
            "errors": [{"code": "save_failed", "message": f"Failed to commit changes atomically: {exc}"}]
        }
...
def get_db_and_model(domain: str) -> tuple[str, str]:
    ...
    db_path = PROJECT_ROOT / "src" / "grc_agent" / "vectors" / db_name
    if not db_path.exists():
        db_path = PROJECT_ROOT / "PydanticAI_experiment" / "vectors" / db_name
...
def embed_query(query: str) -> list[float]:
    ...
    response = client.embeddings.create(
        model=model,
        input="task: search result | query: " + query if not is_openrouter else query
    )
...
def query_catalog(query: str, limit: int = 5) -> dict[str, Any]:
    ...
    db_path, _ = get_db_and_model("catalog")
...
def query_docs(query: str, limit: int = 5) -> dict[str, Any]:
    ...
    db_path, _ = get_db_and_model("docs")
    if not os.path.exists(db_path):
